...
static const struct net_device_ops ls1x_gmac_netdev_ops = {
...
void synopGMAC_linux_powerup_mac(synopGMACdevice *gmacdev)
{
...
int synop_handle_received_data(struct net_device *netdev, int limit)
#else
...
#ifdef CONFIG_GMAC_NAPI
static inline void gmac_schedule(synopGMACPciNetworkAdapter *adapter)
{
	    synopGMACdevice *gmacdev = adapter->synopGMACdev; 
...
{
	    synopGMACPciNetworkAdapter *adapter = container_of(napi, synopGMACPciNetworkAdapter, napi);
		struct net_device *ndev = adapter->synopGMACnetdev;
		synopGMACdevice * gmacdev = adapter->synopGMACdev;;
		int work_done = 0;

		work_done = synop_handle_received_data(ndev, budget);
		synop_handle_transmit_over(ndev);
...
TR("%s called \n",__FUNCTION__);
	adapter = netdev_priv(netdev);
return( &(adapter->synopGMACNetStats) );
}
...
static int gmac_get_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	synopGMACPciNetworkAdapter *adapter = netdev_priv(dev);
...
void * __init synopGMAC_init_network_interface(struct device *dev,u8* synopGMACMappedAddr,u32 synopGMACMappedAddrSize,int irq)
{
	...
	*/
	...
	netdev->netdev_ops	= &ls1x_gmac_netdev_ops;
...
#ifdef CONFIG_GMAC_NAPI
	netif_napi_add(netdev, &synopGMACadapter->napi, gmac_poll, 64);
#endif
