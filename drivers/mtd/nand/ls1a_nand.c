...
static int ls1a_nand_ecc_calculate(struct mtd_info *mtd,
		const uint8_t *dat, uint8_t *ecc_code)
{
	return 0;
}
static int ls1a_nand_ecc_correct(struct mtd_info *mtd,
...
static void ls1a_nand_write_buf(struct mtd_info *mtd,const uint8_t *buf, int len)
{
        int i;
        struct ls1a_nand_info *info = mtd->priv;
...
static int ls1a_nand_verify_buf(struct mtd_info *mtd,const uint8_t *buf, int len)
{
       int i=0;
        struct ls1a_nand_info *info = mtd->priv;
		return 0;
...
static int ls1a_nand_suspend(struct platform_device *pdev)
{
