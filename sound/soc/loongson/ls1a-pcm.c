	...
	struct snd_pcm_hw_params *params)
{
	...
	struct ls1a_pcm_dma_params *dma = rtd->dai->cpu_dai->dma_data;
	int ret;
	...
	if (prtd->params != dma || prtd->params == NULL) {
		...
		ret = request_irq(LS1X_BOARD_DMA1_IRQ, ls1a_pcm_dma_irq, IRQF_SHARED,
		...
		ret = request_irq(LS1X_BOARD_DMA2_IRQ, ls1a_pcm_dma_irq, IRQF_SHARED,
...
struct snd_soc_platform ls1a_soc_platform = {
	.name		= "ls1a-audio",
	.pcm_ops 	= &ls1a_pcm_ops,
...
{
	return snd_soc_register_platform(&ls1a_soc_platform);
}
module_init(ls1a_soc_platform_init);

static void __exit ls1a_soc_platform_exit(void)
{
	snd_soc_unregister_platform(&ls1a_soc_platform);
}
