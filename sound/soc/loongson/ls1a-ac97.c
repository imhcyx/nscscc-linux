...
/*	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		cpu_dai->dma_data = &ls1a_ac97_pcm_stereo_out;
	else
		cpu_dai->dma_data = &ls1a_ac97_pcm_stereo_in;
*/
 ...
 */
struct snd_soc_dai ls1a_ac97_dai[] = {
{
	.name = "ls1a-ac97",
	.id = 0,
	.ac97_control = 1,
...
static int __devinit ls1a_ac97_dev_probe(struct platform_device *pdev)
{
	 ...
	 * and running.
	 */
	printk("ls1a_ac97_dev_probe---->>snd_soc_register_dais\n\n\n");
	return snd_soc_register_dais(ls1a_ac97_dai, ARRAY_SIZE(ls1a_ac97_dai));
}
...
{
	pr_debug("Entered %s\n", __func__);
	return snd_soc_register_dais(ls1a_ac97_dai,
				     ARRAY_SIZE(ls1a_ac97_dai));
	pr_debug("Exit %s\n", __func__);
}
...
{
	pr_debug("Entered %s\n", __func__);
	snd_soc_unregister_dais(ls1a_ac97_dai,
				ARRAY_SIZE(ls1a_ac97_dai));
	pr_debug("Exit %s\n", __func__);
}
