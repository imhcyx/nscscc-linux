...
#include <asm/loongson-ls1x/ls1x_board.h>
...
static int ls1x_audio_sync(struct file *file)
{
	...
	if (file->f_mode & FMODE_READ) 
	{
		...
		if(!list_empty(&is->run_list))
	 	schedule_timeout_uninterruptible(CONFIG_HZ*2);
...
static int ls1x_audio_release(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_READ) {
		...
		state->rd_ref = 0;
		free_irq(LS1X_BOARD_DMA2_IRQ, state->input_stream);
#ifdef CONFIG_SND_LS1X_TIMER
	...
	if (file->f_mode & FMODE_WRITE) {
		...
		state->wr_ref = 0;
		free_irq(LS1X_BOARD_DMA1_IRQ, state->output_stream);
#ifdef CONFIG_SND_LS1X_TIMER
...
static int ls1x_audio_open(struct inode *inode, struct file *file)
{
	...
	if ((file->f_mode & FMODE_WRITE)) {
		...
		request_irq(LS1X_BOARD_DMA1_IRQ, ac97_dma_write_intr, IRQF_SHARED,
				"ac97dma-write", os);
#ifdef CONFIG_SND_LS1X_TIMER
		init_timer(&os->timer);
	...
	if (file->f_mode & FMODE_READ) {
		...
		request_irq(LS1X_BOARD_DMA2_IRQ, ac97_dma_read_intr, IRQF_SHARED,
				"ac97dma-read", is);
#ifdef CONFIG_SND_LS1X_TIMER
		init_timer(&is->timer);
...
static int ls1x_audio_remove(struct platform_device *pdev)
{
	...
	ls1x_ac97_refcount--;
	if (!ls1x_ac97_refcount) {
		free_irq(LS1X_BOARD_AC97_IRQ, &ls1x_audio_state);
	}
	up(&ls1x_ac97_mutex);
...
static int __devinit ls1x_audio_init(void)
{
    ...
    if(ret)
        printk(KERN_ERR "failed to register ls1x-audio drover");
    return ret;
}
